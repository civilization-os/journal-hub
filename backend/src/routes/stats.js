...
router.get('/', (req, res) => {
  try {
    ...
    const recentJournals = db.prepare('SELECT id, title, date, created_at FROM journals ORDER BY created_at DESC LIMIT 5').all();
    ...
    res.json({
      ...
      pending_todos: pendingTodos.map(t => ({ ...t, tags: JSON.parse(t.tags), completed: !!t.completed })),
    });
  } catch (err) {
